...
const searchPokemon= document.getElementById("pokemonInput")
...
const getPostById = async () => {
    ...
    if (searchPokemon) {
        try {
            const response = await fetch(`${apiUrl}/${searchPokemon}`); // Requête GET avec l'ID du post à récupérer
            const data = await response.json(); // Conversion de la réponse en format JSON
            displayResult(data); // Affichage du post spécifié
           
        } catch (error) {
            displayResult({ error: `Erreur lors de la récupération du pokémon ${searchPokemon}}` }); // Gestion des erreurs
        }
    }
};
